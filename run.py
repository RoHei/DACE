...
def prepare_plans(configs):
    ...
    plans_meta = get_plan_encodings(configs, op_name_to_one_hot, plan_parameters, feature_statistics)
    return plans_meta


def train_model(configs):
    # get plan encodings
    plan_encodings = prepare_plans(configs)
    ...
    for plan_encoding in plan_encodings:
        if plan_encoding[-1] in test_database_ids:
            test_data.append(plan_encoding[:-1])
        else:
            train_data.append(plan_encoding[:-1])

    # split train_data into train_data and val_data by 9:1
    train_data, val_data = train_test_split(train_data, test_size=0.1, random_state=configs["random_seed"])
    ...
    # train model
    model = DACELora(configs["node_length"],
                     configs["hidden_dim"],
                     1,
    ...
    trainer = PLTrainer(accelerator="gpu",
                        devices=[0],
...
def train_with_tune(configs):
    result = train_model(configs)
    return {"test_database_ids": configs["test_database_ids"], **result}


def train_with_ray(configs):
    ray_train = tune.with_resources(train_with_tune, {"CPU": 1, "GPU": 0.2})
    tune_config = TuneConfig(max_concurrent_trials=10)
    experiment_spaces = {**configs, "test_database_ids": tune.grid_search(list(range(20)))}
    tuner = tune.Tuner(ray_train, param_space=experiment_spaces, tune_config=tune_config)
    results = tuner.fit()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hidden_dim", type=int, default=128, help="hidden dimension in transformer")
    ...
    parser.add_argument("--test_database_ids", type=int, default=13, nargs="+", help="test database list")
    ...
    parser.add_argument("--mlp_dropout", type=float, default=0.3, help="dropout rate in MLP")
    parser.add_argument("--transformer_dropout", type=float, default=0.2, help="dropout rate in transformer")
    parser.add_argument("--plans_dir", type=str, default="data/training_workloads", help="plans directory")
    parser.add_argument("--statistics_path", type=str, default="data/training_workloads/statistics.json",
                        help="statistics file path")
    parser.add_argument("--process_plans", action="store_true", help="process plans to features")
    parser.add_argument("--test_all", action="store_true", help="whether to test all database (if false, only test the database in test_database_ids)")
    parser.add_argument("--progress_bar", action="store_true",  help="whether to show progress bar in training", )
